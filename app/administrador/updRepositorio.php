...
<?php
    ...
    //HACEMOS CONSULTA DE COMPROBACION PARA TRAER EL USUARIO QUE SE ESTA TRAYENDO
    $stm = $pdo->prepare("SELECT al.usuario FROM repositorios as rp
    inner join alumnos as al on al.idUsuario = rp.id_usuario
    WHERE rp.idRepositorio = :idRepo");
    $stm->execute(array(
        "idRepo"    =>  $idRepositorio
    ));
    $reposit = $stm->fetch(PDO::FETCH_ASSOC);
    $usuario = $reposit['usuario'];
